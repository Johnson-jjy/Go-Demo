...
func newSegment(bucketNumber int, pairRedistributor PairRedistributor) Segment {
	...
	if pairRedistributor == nil {
		pairRedistributor = newDefaultPairRedistributor(DEFAULT_BUCKET_LOAD_FACTORY, bucketNumber)
	}
...
// ? 疑问，return在sUnlock之后，这不会出问题吗？
func (s *segment) GetWithHash(key string, keyHash uint64) Pair {
	s.lock.Lock()
	b := s.buckets[int(keyHash%uint64(s.bucketLen))]
	s.lock.Unlock()
	return b.Get(key)
}
